...
void Aquamarine::CDRMAtomicRequest::planeProps(Hyprutils::Memory::CSharedPointer<SDRMPlane> plane, Hyprutils::Memory::CSharedPointer<CDRMFB> fb, uint32_t crtc,
    ...
    if (!fb || !crtc) {
        ...
        add(plane->id, plane->props.bits.fb_id, 0);
        add(plane->id, plane->props.bits.crtc_id, 0);
        add(plane->id, plane->props.bits.crtc_x, (uint64_t)pos.x);
        add(plane->id, plane->props.bits.crtc_y, (uint64_t)pos.y);
        return;
    }

    TRACE(backend->log(AQ_LOG_TRACE,
                       std::format("atomic planeProps: prop blobs: src_x {}, src_y {}, src_w {}, src_h {}, crtc_w {}, crtc_h {}, fb_id {}, crtc_id {}", plane->props.bits.src_x,
                                   plane->props.bits.src_y, plane->props.bits.src_w, plane->props.bits.src_h, plane->props.bits.crtc_w, plane->props.bits.crtc_h,
                                   plane->props.bits.fb_id, plane->props.bits.crtc_id)));

    // src_ are 16.16 fixed point (lol)
    add(plane->id, plane->props.bits.src_x, 0);
    add(plane->id, plane->props.bits.src_y, 0);
    add(plane->id, plane->props.bits.src_w, ((uint64_t)fb->buffer->size.x) << 16);
    add(plane->id, plane->props.bits.src_h, ((uint64_t)fb->buffer->size.y) << 16);
    add(plane->id, plane->props.bits.crtc_w, (uint32_t)fb->buffer->size.x);
    add(plane->id, plane->props.bits.crtc_h, (uint32_t)fb->buffer->size.y);
    add(plane->id, plane->props.bits.fb_id, fb->id);
    add(plane->id, plane->props.bits.crtc_id, crtc);
...
void Aquamarine::CDRMAtomicRequest::planePropsPos(Hyprutils::Memory::CSharedPointer<SDRMPlane> plane, Hyprutils::Math::Vector2D pos) {
    ...
    TRACE(backend->log(AQ_LOG_TRACE, std::format("atomic planeProps: pos blobs: crtc_x {}, crtc_y {}", plane->props.bits.crtc_x, plane->props.bits.crtc_y)));
...
void Aquamarine::CDRMAtomicRequest::addConnector(Hyprutils::Memory::CSharedPointer<SDRMConnector> connector, SDRMConnectorCommitData& data) {
    ...
    TRACE(backend->log(AQ_LOG_TRACE,
                       std::format("atomic addConnector blobs: mode_id {}, active {}, crtc_id {}, link_status {}, content_type {}", connector->crtc->props.bits.mode_id,
                                   connector->crtc->props.bits.active, connector->props.bits.crtc_id, connector->props.bits.link_status, connector->props.bits.content_type)));
    ...
    add(connector->id, connector->props.bits.crtc_id, enable ? connector->crtc->id : 0);

    if (enable && connector->props.bits.content_type)
        add(connector->id, connector->props.bits.content_type, STATE.contentType);

    add(connector->crtc->id, connector->crtc->props.bits.active, enable);

    if (enable) {
        if (connector->output->supportsExplicit && STATE.committed & COutputState::AQ_OUTPUT_STATE_EXPLICIT_OUT_FENCE)
            add(connector->crtc->id, connector->crtc->props.bits.out_fence_ptr, (uintptr_t)&STATE.explicitOutFence);

        if (connector->crtc->props.bits.gamma_lut && data.atomic.gammad)
            add(connector->crtc->id, connector->crtc->props.bits.gamma_lut, data.atomic.gammaLut);

        if (connector->crtc->props.bits.degamma_lut && data.atomic.degammad)
            add(connector->crtc->id, connector->crtc->props.bits.degamma_lut, data.atomic.degammaLut);

        if (connector->crtc->props.bits.ctm && data.atomic.ctmd)
            add(connector->crtc->id, connector->crtc->props.bits.ctm, data.atomic.ctmBlob);

        if (connector->crtc->props.bits.vrr_enabled)
            add(connector->crtc->id, connector->crtc->props.bits.vrr_enabled, (uint64_t)STATE.adaptiveSync);
        ...
        if (connector->output->supportsExplicit && STATE.explicitInFence >= 0)
            add(connector->crtc->primary->id, connector->crtc->primary->props.bits.in_fence_fd, STATE.explicitInFence);

        if (connector->crtc->primary->props.bits.fb_damage_clips)
            add(connector->crtc->primary->id, connector->crtc->primary->props.bits.fb_damage_clips, data.atomic.fbDamage);
    } else {
...
void Aquamarine::CDRMAtomicRequest::addConnectorModeset(Hyprutils::Memory::CSharedPointer<SDRMConnector> connector, SDRMConnectorCommitData& data) {
    ...
    add(connector->crtc->id, connector->crtc->props.bits.mode_id, enable ? data.atomic.modeBlob : 0);
    data.atomic.blobbed = true;

    if (!enable)
        return;

    if (connector->props.bits.link_status)
        add(connector->id, connector->props.bits.link_status, DRM_MODE_LINK_STATUS_GOOD);

    if (connector->props.bits.max_bpc && connector->maxBpcBounds.at(1))
        add(connector->id, connector->props.bits.max_bpc, getMaxBPC(data.mainFB->buffer->dmabuf().format));

    if (connector->props.bits.Colorspace && connector->colorspace.bits.BT2020_RGB)
        add(connector->id, connector->props.bits.Colorspace, STATE.wideColorGamut ? connector->colorspace.bits.BT2020_RGB : connector->colorspace.bits.Default);

    if (connector->props.bits.hdr_output_metadata && data.atomic.hdrd)
        add(connector->id, connector->props.bits.hdr_output_metadata, data.atomic.hdrBlob);
}
...
bool Aquamarine::CDRMAtomicImpl::prepareConnector(Hyprutils::Memory::CSharedPointer<SDRMConnector> connector, SDRMConnectorCommitData& data) {
    ...
    if (STATE.committed & COutputState::AQ_OUTPUT_STATE_GAMMA_LUT)
        data.atomic.gammad = prepareGammaBlob(connector->crtc->props.bits.gamma_lut, STATE.gammaLut, &data.atomic.gammaLut);

    if (STATE.committed & COutputState::AQ_OUTPUT_STATE_DEGAMMA_LUT)
        data.atomic.degammad = prepareGammaBlob(connector->crtc->props.bits.degamma_lut, STATE.degammaLut, &data.atomic.degammaLut);

    if ((STATE.committed & COutputState::AQ_OUTPUT_STATE_CTM) && data.ctm.has_value()) {
        if (!connector->crtc->props.bits.ctm)
    ...
    if ((STATE.committed & COutputState::AQ_OUTPUT_STATE_HDR) && data.hdrMetadata.has_value()) {
        if (!connector->props.bits.hdr_output_metadata)
    ...
    if ((STATE.committed & COutputState::AQ_OUTPUT_STATE_DAMAGE) && connector->crtc->primary->props.bits.fb_damage_clips && MODE) {
...
bool Aquamarine::CDRMAtomicImpl::reset() {
    ...
    for (auto const& crtc : backend->crtcs) {
        request.add(crtc->id, crtc->props.bits.mode_id, 0);
        request.add(crtc->id, crtc->props.bits.active, 0);
    }

    for (auto const& conn : backend->connectors) {
        request.add(conn->id, conn->props.bits.crtc_id, 0);
    }
